...
module Config =
    ...
    module Consumer =
        ...
        module Topic =
            ...
            let autoOffsetReset (reset: AutoOffsetReset) = config<string> "auto.offset.reset" (enumToString reset)
    ...
    let stringConfig (config: Map<string,obj>) =
      config |> Map.map(fun _ v -> v.ToString())
...
module Producer =
  ...
  /// Creates a producer.
  let create (config: Map<string,string>) =
    let producer = new Producer<'TKey,'TValue>(config)
...
module Consumer =
  ...
  let create (config: Map<string,string>) =
    let consumer = new Consumer<'TKey,'TValue>(config)
